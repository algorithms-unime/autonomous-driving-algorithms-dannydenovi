...
def train(step):
    env = gym.make("CarRacing-v2", domain_randomize=False, verbose=1)
...
def retrain(step, loaded_model):
    ...
    model = PPO.load(loaded_model , env, verbose=1, tensorboard_log="./tensorboard/")
    model.learn(total_timesteps=step, reset_num_timesteps=False, tb_log_name="ppo")
...
def test(loaded_model):
    ...
    model = PPO.load(loaded_model, env, verbose=1)
    obs, _ = env.reset()
    done = False
    while not done:
        env.render()
